    ...
    if(token) {
        try {
            ...
            if (Number.isInteger(result)) {
                res.jsend.success({"result": result, "error": err});
            }
            else {
                res.jsend.success({"result": Math.round(result), "error": err, "message": "Result has been rounded, as it was not an integer."});
            }
        }
    }
    if (Number.isInteger(result)) {
